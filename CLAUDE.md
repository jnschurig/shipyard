...
**Per-game pluggable model.** Everything game-specific is behind the `games::Game` trait (`src/games/mod.rs`). Two implementations ship today: SoH (`src/games/soh.rs`, dual OoT / OoT-MQ slots, per-install cached `.o2r`) and 2Ship (`src/games/twoship.rs`, single Majora's Mask slot, **user-global** cached archive under `ProjectDirs("com","2ship2harkinian","2s2h")`, AppImage nested inside a release zip). `games::registry()` enumerates them; the UI exposes a runtime game picker via `App.selected_game_slug`, and installs are partitioned on disk by game slug (`<library_root>/<game_slug>/<tag>/`). When adding a game-specific behavior, the answer is almost always "add it to the `Game` trait", not "branch on game slug".
...
**ROM library + launch wiring.** Imported ROMs are plain copies in `<config_dir>/roms/<original_filename>` (no hashing, no validation, no format detection — trust the user's pick). Slot assignments map a slot to a ROM filename. At launch, `roms::wiring::reconcile` places `<install_dir>/<slot.symlink_filename>` as a symlink to the assigned ROM in the library — **unconditionally**, every launch, for every assigned slot. An earlier version skipped this when the cached `.o2r`/`.otr` was already present, but that broke 2Ship: its cached archive lives in a user-global path, so a stale archive can trigger a regenerate-from-ROM prompt and the ROM must be findable in the install dir at that moment. The ~32MB-equivalent cost is cheap vs. the failure mode. Symlinks are placed atomically (symlink → rename) and skipped only when the existing link already points at the right target. Launch passes no ROM-related CLI args; the symlink is the only wiring mechanism.

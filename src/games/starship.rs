...
mod tests {
    ...
    fn extract_linux_unzips_full_release_tree() {
        // Starship ships a flat zip with appimage + assets/ + config.yml +
        // gamecontrollerdb.txt at root. All of it must land in the install dir
        // or the game crashes in AudioLoad_Init.
        ...
        w.start_file("starship.appimage", opts).unwrap();
        w.write_all(b"appimage-body").unwrap();
        w.start_file("config.yml", opts).unwrap();
        w.write_all(b"k: v").unwrap();
        w.start_file("assets/yaml/us/rev0/ast_audio.yaml", opts)
            .unwrap();
        w.write_all(b"audio: data").unwrap();
        ...
        assert!(dest.join("assets/yaml/us/rev0/ast_audio.yaml").exists());
